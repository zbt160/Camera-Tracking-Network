...
def main(cam1_path, cam2_path):
	...
	while True:
		...
		#print('cam1_personsTracked bbox:')
		#print(cam1_personsTracked)
		
		
		aggregatedPersonsList = centralServer(cam1_personsTracked, cam2_personsTracked, cam1_image, cam2_image)
		
		if(len(aggregatedPersonsList)):
			#Draw tracks and bbox for individual camera outputs and aggregated outputs
			print('aggregatedPersonsList')
...
def personTracking(image, tracker):
...
def modifyTracker(tracker, persons, threshold_centroid):
	#tracker contains 
	present = False
	newlist = list(tracker)
	for (x,y,w,h) in persons:
		centroid_person = get_centroid(x,y,w,h)
		present = False
		for (xp,yp,wp,hp) in tracker:
...
def check_threshold(centroid_tracker,centroid_person,threshold_centroid):
	...
	if(distance < threshold_centroid):
		...
		return False
'''		
def centralServer(cam1_persons, cam2_persons, image_cam1, image_cam2):
	#From the individual persons list get aggregated list
	print('\tcentralServer')
	personsList = []
	return personsList
'''
...
def centralServer(cam1_persons, cam2_persons, cam1_image, cam2_image):
	'''
	Description:

	Input:
		cam1_persons: list of objects, from camera 1, of type rect2D (x,y, w, h) 
									where x,y = pixel location & w,h = width, height
		cam2_persons: list of objects, from camera 2, of type rect2D (x,y, w, h) 
									where x,y = pixel location & w,h = width, height
		cam1_image:			image frame at time, t, from camera 1
		cam2_image:			image frame at time, t, from camera 2
	Ouput:
		unique_persons: a list of objects, of type person 
										containing global IDs for all unique people tracked
...
def cam_personTracking(image, tracker, flag):
	...
	personsMod = personsTracked
	if(flag):
		personsDetected = personDetection(image)
		if personsDetected:
			personsMod = modifyTracker(personsTracked, personsDetected, threshold_centroid)
