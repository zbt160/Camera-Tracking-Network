...
def main(cam1_path, cam2_path):
	...
	while True:
		...
		cam1_personsTracked, cam1_ID, new_flag_1 = cam_personTracking(cam1_image, cam1_tracker, personDetectionFlag, cam1_ID)#True)#personDetectionFlag)
		cam2_personsTracked, cam2_ID, new_flag_2 = cam_personTracking(cam2_image, cam2_tracker, personDetectionFlag, cam2_ID)#True)#personDetectionFlag)
		...
		#aggregatedPersonsList = centralServer(cam1_personsTracked, cam2_personsTracked, cam1_image, cam2_image, cam1_ID, cam2_ID, new_flag_1, new_flag_2)
...
def modifyTracker(tracker, persons, threshold_centroid, c_ID, new):#tracked, detected, threshold_centroid
	#tracker contains 
	present = False
	...
	for (x,y,w,h) in persons:#For every detected person in frame
		...
		if (present == False): #this is a new person
			...
			if (len(c_ID) ==0) :
				...
				c_ID.append(c_ID[-1] + 1)

	return newlist, c_ID, new
...
###############################################################
def cam_personTracking(image, tracker, flag, c_ID):
	#print('\tcam_personTracking')
	threshold_centroid = 10
	new_flag = False
	...
	if(flag): #Every tenth frame
		...
		if personsDetected: #if frame not empty
			personsMod, c_ID, new_flag = modifyTracker(personsTracked, personsDetected, threshold_centroid, c_ID, new_flag)
			#print('\tAm I a List?: ')
	...
	return personsMod, c_ID, new_flag
