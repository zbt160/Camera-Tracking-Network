...
def main(cam1_path, cam2_path):
	...
	while True:
		...
		if(frameNumber == 1 or frameNumber%personDetectionFrameThres == 0):
			...
			personDetectionFlag = False
		gamma = 1.5
		cam1_image = gammaCorrection(cam1_image, gamma)
		cam2_image = gammaCorrection(cam2_image, gamma)
...
def personDetection(image, th):
	...
	#hog.setSVMDetector(cv2.HOGDescriptor_getPeopleDetector48x96())#getDaimlerPeopleDetector
	#(personsDetected, weights) = hog.detectMultiScale(image, winStride=(4, 4), padding=(8, 8), scale=1.05)
	(personsDetected, weights) = hog.detectMultiScale(image, winStride=(4, 4), padding=(4, 4), scale=1.02)
	#print(personsDetected)
	personsDetected_new = []
	#Threshold being added to return only the person in the image and to remove background
	local_th = 10
	for (x,y,w,h) in personsDetected:
		#personsDetected_new.append([x+th,y+th,w,h])
		personsDetected_new.append([x+th+local_th,y+th+local_th,w-local_th,h-local_th])
...
def cam_personTracking(image, tracker, flag):
	...
	threshold_centroid = 30#60
	th = 5#40#boundaryThreshold_centroid
	H, W, C = image.shape
...
if __name__ == '__main__' :
	...
	arg1 = "Dataset\\videoRPI-new240-0.mp4";
	arg2 = "Dataset\\videoRPI-new240-1.mp4";
